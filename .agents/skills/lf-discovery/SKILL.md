---
name: lf-discovery
description: "Interactive feature discovery. Conducts structured 7-phase interview: collects reference documents (URLs or pasted), extracts inputs, determines greenfield vs brownfield, asks gap questions, researches market benchmarks, generates discovery.md. Works with any project (Angular, Go, Python, etc.). Use for new features, planning initiatives, or /lf-discovery."
metadata:
...
**Se Brownfield:**
- Use a ferramenta Read para ler `BRJOBS.md`, `CLAUDE.md`, ou similar documento de contexto do projeto (se existir). Extraia stack e padrões.
- Use a ferramenta Glob para verificar se há `package.json`, `go.mod`, `pom.xml`, `pyproject.toml` e confirme a stack.
- Use a ferramenta Glob para listar specs anteriores em `ai/specs/` como referência de padrão interno.
- Informe ao usuário: "Encontrei que o projeto usa [stack]. Li o contexto de padrões e vou trabalhar com essas convenções: [breve resumo]."
- Faça as perguntas abaixo (agrupe as relacionadas em uma mensagem):
  ...
  - "Quais padrões internos devem ser seguidos? (autenticação, integrações, camadas arquiteturais...)"
