...
- aplicar anti-spam moderado no backend (rate limit por usuário em memória, sem infra paga extra);
...
- rate limit distribuído com Redis/ElastiCache (fica para fase de escala);
...
- **Anti-spam equilibrado (MVP):** bloquear abuso sem tornar chat rígido para usuário legítimo.
- **Política inicial anti-spam:** máximo de 20 mensagens por 60 segundos por usuário, intervalo mínimo de 800 ms entre envios e bloqueio de mensagem duplicada em sequência por 15 segundos.
- **Custo AWS baixo:** anti-spam implementado em memória por instância da API, sem dependência de serviços gerenciados adicionais nesta fase.
...
[Usuário logado]
   ...
   -> BE valida anti-spam (intervalo, janela e duplicidade)
   -> se exceder política, BE retorna 429 sem persistir
...
| POST | /api/v1/chat/enviar?destinatarioId={id} | Sim | `{ conteudo, publicacaoId?, clientTempId? }` | `MessageItem` (ou `429 Too Many Requests` em anti-spam) |
...
- proteção anti-spam deve ocorrer na camada de Service, antes de persistir mensagem;
...
Referências visuais mapeadas para esta feature:

- Tela de Chat (`/chat`)
   > **Figma:** ⚠️ Referência visual pendente — solicitar ao time de UX/UI
- Header com sino de notificações (global)
   > **Figma:** ⚠️ Referência visual pendente — solicitar ao time de UX/UI
- Perfil público (entrada para chat em `/perfil/:usuarioId`)
   > **Figma:** ⚠️ Referência visual pendente — solicitar ao time de UX/UI
- Detalhe da publicação (entrada para chat em `/publicacoes/:id`)
   > **Figma:** ⚠️ Referência visual pendente — solicitar ao time de UX/UI
...
11. Sistema DEVE aplicar anti-spam moderado por remetente com janela deslizante de curto prazo.
12. SE remetente exceder o limite configurado, ENTÃO o backend DEVE responder `429 Too Many Requests` com mensagem orientativa.
13. Sistema NÃO DEVE aplicar bloqueio prolongado (banimento) por anti-spam no MVP; bloqueio deve ser apenas temporário e de baixa fricção.
14. Sistema DEVE bloquear repetição imediata de texto idêntico em sequência dentro da janela configurada.

### 7.1 Política anti-spam recomendada (MVP)

| Regra | Valor inicial | Objetivo |
|------|---------------|----------|
| Máximo por janela | 20 mensagens / 60 segundos por usuário | Reduzir flood sem atrapalhar conversa normal |
| Intervalo mínimo | 800 ms entre envios do mesmo usuário | Evitar rajadas por clique repetido/automação simples |
| Duplicidade | Bloquear mesma mensagem em sequência por 15 segundos | Evitar spam repetitivo de texto |

Diretriz de produto para tuning:

- priorizar redução de falso positivo (usuário legítimo bloqueado);
- ajustar limites por configuração, sem novo deploy;
- revisar bloqueios em produção nos primeiros 7 dias pós-go-live.
...
| Rate limit anti-spam acionado | excesso de envios em janela curta ou repetição imediata | rejeitar com 429, não persistir, permitir nova tentativa após curto intervalo | Você está enviando mensagens muito rápido. Aguarde alguns segundos. |
...
| chat_rate_limit_blocked | user_id, destinatario_id, reason(interval/window/duplicate), timestamp | warn |
...
| chat_rate_limit_block_rate | taxa | percentual de envios bloqueados por anti-spam |
| chat_duplicate_block_rate | taxa | percentual de bloqueios por mensagem duplicada |
...
| chat_rate_limit_block_rate alto | > 8% por 15 min | revisar tuning para evitar política excessivamente restritiva |
...
✱ CHAT_RATE_LIMIT_ENABLED=true           # BE — liga/desliga proteção anti-spam
✱ CHAT_RATE_LIMIT_MAX_MESSAGES=20        # BE — máximo de mensagens por janela
✱ CHAT_RATE_LIMIT_WINDOW_SECONDS=60      # BE — janela do rate limit em segundos
✱ CHAT_RATE_LIMIT_MIN_INTERVAL_MS=800    # BE — intervalo mínimo entre envios por usuário
✱ CHAT_RATE_LIMIT_DUPLICATE_WINDOW_SECONDS=15  # BE — bloqueio de duplicata em sequência
...
3. Smoke test end-to-end:
   ...
   - validar bloqueio anti-spam sem impedir conversa normal;
   - validar badge no header;
   - validar marcação como lida;
4. Monitorar métricas/erros por janela inicial de estabilização.
5. Ajustar thresholds de anti-spam por configuração (sem deploy) se houver falso positivo.
...
- [ ] Implementar anti-spam moderado no backend com resposta 429 para abuso.
...
| R06 | Spam de mensagens por usuário malicioso | Média | Médio | Aplicar rate limit moderado (janela + intervalo + duplicata) e revisar block rate no pós-go-live |
| R07 | Política anti-spam ficar rígida demais e bloquear usuário legítimo | Média | Médio | Iniciar com limites moderados e ajustar variáveis de ambiente sem deploy |
...
**Pontos em aberto (não bloqueadores para MVP):**

- ℹ️ **Escala horizontal futura:** quando API rodar com múltiplas réplicas, avaliar migração do anti-spam para store distribuído (Redis/ElastiCache) para consistência global por usuário.

---

## 16. Referências de Design (Figma)

> Gerado por /lf-specs em 2026-04-10. URLs fornecidas pelo time de UX/UI.
...
|------|------|-----------|-------------------|--------|
| Tela de Chat | /chat | — | Lista de conversas, histórico, composer de mensagem | ⚠️ Pendente |
| Header com sino de não lidas | Global | — | Ícone de sino, badge de contagem, estado zerado | ⚠️ Pendente |
| Perfil público (entrada de chat) | /perfil/:usuarioId | — | CTA de contato, navegação para chat com query params | ⚠️ Pendente |
| Detalhe da publicação (entrada de chat) | /publicacoes/:id | — | CTA de contato, navegação para chat com contexto da publicação | ⚠️ Pendente |
